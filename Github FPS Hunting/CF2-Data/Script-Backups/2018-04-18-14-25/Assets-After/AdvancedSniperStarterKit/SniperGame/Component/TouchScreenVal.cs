...
public class TouchScreenVal
{
	...
	public void Update ()
	{
		for (var i = 0; i < ControlFreak2.CF2Input.touchCount; ++i) {
			if(ControlFreak2.CF2Input.GetTouch (i).phase == TouchPhase.Canceled || ControlFreak2.CF2Input.GetTouch (i).phase == TouchPhase.Ended){
				IsPressed = false;
			}
		}
		Initial ();
	...
	public bool OnTouchPress ()
	{
		...
		bool res = false;
		for (var i = 0; i < ControlFreak2.CF2Input.touchCount; ++i) {
			Vector2 touchpos = ControlFreak2.CF2Input.GetTouch (i).position;
			if (ControlFreak2.CF2Input.GetTouch (i).phase == TouchPhase.Began || ControlFreak2.CF2Input.GetTouch (i).phase == TouchPhase.Stationary) {
	...
	public bool OnTouchRelease ()
	{
		...
		for (var i = 0; i < ControlFreak2.CF2Input.touchCount; ++i) {
			Vector2 touchpos = ControlFreak2.CF2Input.GetTouch (i).position;
			if (ControlFreak2.CF2Input.GetTouch (i).phase == TouchPhase.Ended || ControlFreak2.CF2Input.GetTouch (i).phase == TouchPhase.Canceled) {
				if ((touchpos.x >= AreaTouch.xMin && touchpos.x <= AreaTouch.xMax && Screen.height - touchpos.y >= AreaTouch.yMin && Screen.height - touchpos.y <= AreaTouch.yMax)) {
					res = true;
					IsPressed = true;
				}
	...
	public Vector2 OnTouchDirection (bool fixdrag)
	{
		...
		for (var i = 0; i < ControlFreak2.CF2Input.touchCount; ++i) {
			Vector2 touchpos = ControlFreak2.CF2Input.GetTouch (i).position;
			if ((touchpos.x >= AreaTouch.xMin && touchpos.x <= AreaTouch.xMax && Screen.height - touchpos.y >= AreaTouch.yMin && Screen.height - touchpos.y <= AreaTouch.yMax)) {
				if (ControlFreak2.CF2Input.GetTouch (i).phase == TouchPhase.Began) {
					controllerPositionNext = new Vector2 (ControlFreak2.CF2Input.GetTouch (i).position.x, Screen.height - ControlFreak2.CF2Input.GetTouch (i).position.y);
