...
//	void OnEnable(){
//		if (AmmoText1 != null)
//		AmmoText1.uiText = GetComponentInChildren<UnityEngine.UI.Text> ();  
//	}
...
//				}
	...
	void HitObject ( RaycastHit hit, Vector3 directionArg, bool isSecondCast = false ){
		...
		case 13://hit object is an NPC
			...
			if(hit.collider.gameObject.GetComponent<Emerald_Animal_AI>()){
				hit.collider.gameObject.GetComponent<Emerald_Animal_AI>().Damage((int)damageAmt);
				hit.collider.gameObject.GetComponent<Emerald_Animal_AI>().MakeFlee(FPSPlayerComponent.gameObject);
				hit.collider.gameObject.GetComponent<Emerald_Animal_AI>().MakeAttack(FPSPlayerComponent.gameObject);
			}
